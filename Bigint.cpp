...
namespace myBigint
{
	...
	namespace FFT
	{
		...
		void init(int n, int l)
		{
			...
			for(int i = 0; i < half; i++)
				w1[i | half] = complex<double>(cos(PI * i / half), sin(PI * i / half));
	...
	istream& operator >> (istream& in, Bigint &s)
	{
		...
		for(int j = c.size() - 1; j >= (s.size() - 1) * DIG; j--)
			s.v[s.size() - 1] = 10 * s[s.size() - 1] + c[c.size() - 1 - j] - '0';
		return in;
	}
	...
	Bigint operator + (const Bigint &a, const Bigint &b)
	{
		Bigint ret;
		if(a.sign == b.sign)
			ret = plus_abs(a, b), ret.sign = a.sign;
		else
		{
			if(cmp_abs(a, b))
				ret = minus_abs(b, a), ret.sign = b.sign;
			else
				ret = minus_abs(a, b), ret.sign = a.sign;
		}
		return ret;
	}
	...
	Bigint multiply_abs_fft(const Bigint &a, const Bigint &b)
	{
		...
		for(int i = 0; i < n; i++)
			B[i] = conj(A[(n - i) % n]);
		for(int i = 0; i < n; i++)
			C[i] = (A[i] * A[i] - B[i] * B[i]) / (4.0 * complex<double>(0, 1));
		FFT::IDFT(C);
		ret.resize(n / 3 + 1);
	...
	Bigint multiply_abs_force(const Bigint &a, long long b)
	{
		...
		ret.resize(a.size() + 1);
		for(int i = 0; i < a.size(); i++)
			ret[i] += a[i] * b, ret.v[i + 1] += ret.v[i] / R, ret.v[i] %= R;
		while(ret.size() && !ret[ret.size() - 1])
			ret.pop_back();
		return ret;
	}
	...
	{
		Bigint ret;
		if((long long)a.size() * b.size() < 40000)
			ret = multiply_abs_force(a, b);
		else
			ret = multiply_abs_fft(a, b);
	...
	pair<Bigint, Bigint> divide_abs(const Bigint &a, const Bigint &b)
	{
		...
		for(int i = ret1.size() - 1; i >= 0; i--)
		{
			...
			if(!shift_cmp_abs(ret2, b, i))
				ret2 = shift_minus_abs(ret2, b, i), k++;
			ret1[i] += k;
		}
		while(ret1.size() && !ret1[ret1.size() - 1])
			ret1.pop_back();
		while(ret2.size() && !ret2[ret2.size() - 1])
			ret2.pop_back();
